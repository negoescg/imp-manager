  ...
  const processData = (products: ProductDetail[]) => {
    return products.map((product) => {
      const variantQuantities = {
        ...
        name: '',
        type: '',
        no_jewel: { sku: '', required: 0, completed: 0, stock: 0 } as VariantCell,
        ...
        kids: { sku: '', required: 0, completed: 0, stock: 0 } as VariantCell,
      ...
      if (!product.isBespoke) {
        product.variants?.forEach((variant) => {
          ...
          switch (description) {
            ...
            case 'Earrings':
              ...
              break;
            case 'No Jewel':
              variantQuantities.no_jewel.sku = sku;
              variantQuantities.no_jewel.required = required;
              variantQuantities.no_jewel.completed = completed;
              variantQuantities.no_jewel.stock = took_from_stock;
              variantQuantities.type = type ?? '';
              break;
            default:
              if (variant.isBespoke) {
                variantQuantities.kids.sku = sku;
                variantQuantities.kids.required = required;
                variantQuantities.kids.completed = completed;
                variantQuantities.kids.stock = took_from_stock;
                variantQuantities.type = type ?? '';
              }
              break;
          }
        ...
        variantQuantities.kids.required += product.required ?? 0;
        variantQuantities.kids.sku = product.sku ?? '';
        variantQuantities.kids.completed = product.completed ?? 0;
        variantQuantities.kids.stock = product.took_from_stock ?? 0;
      ...
      case 'no_jewel':
        switch (options.summaryProcess) {
          case 'start':
            ...
            break;
          case 'calculate':
            options.totalValue += options.value.no_jewel.required;
            break;
          case 'finalize':
            break;
        }
      ...
      case 'kids':
        switch (options.summaryProcess) {
          case 'start':
            ...
            break;
          case 'calculate':
            options.totalValue += options.value.kids.required;
            break;
          case 'finalize':
            break;
        }
  ...
  return (
    <DataGrid
      ...
      <Summary calculateCustomSummary={calculateTotals}>
        ...
        <TotalItem summaryType="custom" showInColumn="no_jewel" displayFormat="{0}" name="no_jewel" />
        <TotalItem summaryType="custom" showInColumn="kids" displayFormat="{0}" name="kids" />
