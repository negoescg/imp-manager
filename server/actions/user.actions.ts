...
import { validateAuthFormData, setAuthCookie, genericError, getUserAuth } from '@/lib/auth/utils';
import { users, updateUserSchema } from '@/lib/schema';
...
export async function signInAction(_: ActionResult, formData: FormData): Promise<ActionResult> {
  ...
  try {
    const [existingUser] = await db.select().from(users).where(eq(users.email, data.email.toLowerCase()));
    if (!existingUser) {
      return {
...
export async function updateUser(_: any, formData: FormData): Promise<ActionResult & { success?: boolean }> {
